...
app.get('/api/ice-servers', (_req, res) => {
  ...
  res.json({ iceServers: urls.map(urls => ({ urls })) });
});
...
function relay(socket, event, payload) { const roomId = socket.data.roomId; if (roomId) socket.to(roomId).emit(event, payload); }
